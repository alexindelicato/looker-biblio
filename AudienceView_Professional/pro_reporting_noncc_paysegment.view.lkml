 view: pro_reporting_noncc_paysegment {
   derived_table: {
     sql: SELECT
order0_.client_id as order_payment_nonCC_segment_client_id,
sum(paymentseg1_.amount) as sum_payment_nonCC_segment_amount
...
group by order0_.client_id

       ;;
   }

   dimension: order_payment_nonCC_segment_client_id {
     type: number
     sql: ${TABLE}.order_payment_nonCC_segment_client_id ;;
   }

   dimension: sum_payment_nonCC_segment_amount {

     type: number
     sql: ${TABLE}.sum_payment_nonCC_segment_amount ;;
   }
}
