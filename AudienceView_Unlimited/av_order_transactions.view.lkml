...
UNION ALL
          ...
          SELECT
            ...
            SUM( CASE
              WHEN orderitem_sale_action in ( 0, 1, 2, 4, 5, 6 )
                THEN orderitem_gross_sold_amount
                ELSE 0
            END) as sold_amount,
            SUM( CASE
              WHEN orderitem_sale_action in ( 3 )
                THEN orderitem_gross_sold_amount
                ELSE 0
            END) as returned_amount,
            ...
            SUM( CASE
              WHEN orderitem_sale_action = 1
                THEN orderitem_sold_volume
                ELSE 0
            END) as sold_volume,
            SUM( CASE
              WHEN orderitem_sale_action = 3
                THEN orderitem_sold_volume
                ELSE 0
            END) as returned_volume,
...
UNION ALL
          ...
          SELECT
            ...
            SUM( CASE
              WHEN gift_certificate_sale_action in ( 0, 1, 2, 4, 5, 6 )
                THEN gift_certificate_net_sold_amount
                ELSE 0
            END) as sold_amount,
            SUM( CASE
              WHEN gift_certificate_sale_action in ( 3 )
                THEN gift_certificate_net_sold_amount
                ELSE 0
            END) as returned_amount,

            SUM(gift_certificate_net_sold_amount) as net_sold_amount,

            SUM( CASE
              WHEN gift_certificate_sale_action = 1
                THEN gift_certificate_sold_volume
                ELSE 0
            END) as sold_volume,
            SUM( CASE
              WHEN gift_certificate_sale_action = 3
                THEN gift_certificate_sold_volume
                ELSE 0
            END) as returned_volume,
            SUM( gift_certificate_sold_volume ) as net_volume,

          FROM audienceview.unlimited_certificate_transactions
          WHERE ( gift_certificate_sold_volume <> 0 OR gift_certificate_net_sold_amount <> 0 )
...
UNION ALL
          ...
          SELECT
            ...
            SUM( CASE
              WHEN ordergift_sale_action in ( 0, 1, 2, 4, 5, 6 )
                THEN ordergift_sold_amount
                ELSE 0
            END) as sold_amount,
            SUM( CASE
              WHEN ordergift_sale_action in ( 3 )
                THEN ordergift_sold_amount
                ELSE 0
            END) as returned_amount,
            ...
            SUM( CASE
              WHEN ordergift_sale_action = 1
                THEN ordergift_sold_volume
                ELSE 0
            END) as sold_volume,
            SUM( CASE
              WHEN ordergift_sale_action = 3
                THEN ordergift_sold_volume
                ELSE 0
            END) as returned_volume,
